...
const swaggerOptions = {
  definition: {
    openapi: '3.0.0',
    info: {
      title: process.env.API_TITLE ?? 'Calendar Management API',
...
// Homepage that redirects to Swagger documentation
app.get('/', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
      <head>
        <title>${process.env.API_TITLE ?? 'Calendar Management API'}</title>
        <meta charset="utf-8"/>
      ...
      <body>
        <div class="container">
          <div class="header">
            ...
            <h1>${process.env.API_TITLE ?? 'Calendar Management API'}</h1>
